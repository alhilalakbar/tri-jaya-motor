...
<div class="card mb-3 shadow-sm border-0">
    <div class="card-body">
        <form action="<?= base_url('backend/laporan/laba-rugi'); ?>" method="get" class="row g-3 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-6">
                <div class="btn-group me-2">
                    ...
                    <a href="<?= base_url('backend/laporan/laba-rugi'); ?>" class="btn btn-secondary shadow-sm"><i class="bi bi-arrow-clockwise"></i> Reset</a>
                </div>
                <div class="btn-group">
...
<div class="row">
    <div class="col-md-7">
        <table class="table table-bordered align-middle shadow-sm">
            <tr>
                ...
                <td class="text-end text-success"><strong>Rp <?= number_format($laba->total_pendapatan, 0, ',', '.'); ?></strong></td>
            </tr>
            <tr>
                <th class="bg-light">HPP Sparepart (Modal Barang Terjual)</th>
                <td class="text-end text-danger">Rp <?= number_format($laba->total_pembelian, 0, ',', '.'); ?></td>
            </tr>
            <tr>
                <th class="bg-light">Total Biaya Operasional</th>
                <td class="text-end text-danger">Rp <?= number_format($laba->total_operasional, 0, ',', '.'); ?></td>
            </tr>
            <tr>
            ...
            <tr class="table-primary border-primary">
                ...
                <td class="text-end py-3">
                    <h4 class="m-0 text-primary"><strong>Rp <?= number_format($laba->estimasi_laba_bersih, 0, ',', '.'); ?></strong></h4>
                </td>
            </tr>
        </table>
    </div>
    <div class="col-md-5">
        <div class="alert alert-info border-0 shadow-sm h-100">
            ...
            <hr>
            <p class="small">Laporan ini menggunakan data <strong>real-time</strong> berdasarkan transaksi yang sudah lunas.</p>
            <ul class="small">
...
<script>
    function setRange(type) {
        ...
        if (type === 'today') {
            startInput.value = dateStr; endInput.value = dateStr;
        } else if (type === 'month') {
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 2).toISOString().split('T')[0];
            startInput.value = startOfMonth; endInput.value = dateStr;
        }
    }
</script>
</div>
