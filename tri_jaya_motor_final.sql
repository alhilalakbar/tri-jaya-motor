...
--
-- Host: localhost
-- Generation Time: May 14, 2026 at 11:22 AM
...
,`total_pembelian` decimal(44,2)
,`total_operasional` decimal(34,2)
,`total_gaji` decimal(34,2)
,`estimasi_laba_bersih` decimal(47,2)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`hillal`@`localhost` SQL SECURITY DEFINER VIEW `view_laporan_laba_rugi`  AS SELECT (select coalesce(sum(`ts`.`total_biaya`),0) from `transaksi_servis` `ts` where (`ts`.`status_pembayaran` = 'Lunas')) AS `total_pendapatan`, (select coalesce(sum((`dpp`.`jumlah_pakai` * `s`.`harga_modal`)),0) from ((`detail_penggunaan_part` `dpp` join `sparepart` `s` on((`dpp`.`id_part` = `s`.`id_part`))) join `transaksi_servis` `ts` on((`dpp`.`id_transaksi` = `ts`.`id_transaksi`))) where (`ts`.`status_pembayaran` = 'Lunas')) AS `total_pembelian`, (select coalesce(sum(`bo`.`nominal`),0) from `biaya_operasional` `bo`) AS `total_operasional`, (select coalesce(sum(`ghm`.`nominal`),0) from `gaji_harian_mekanik` `ghm`) AS `total_gaji`, ((((select coalesce(sum(`transaksi_servis`.`total_biaya`),0) from `transaksi_servis` where (`transaksi_servis`.`status_pembayaran` = 'Lunas')) - (select coalesce(sum((`dpp`.`jumlah_pakai` * `s`.`harga_modal`)),0) from ((`detail_penggunaan_part` `dpp` join `sparepart` `s` on((`dpp`.`id_part` = `s`.`id_part`))) join `transaksi_servis` `ts` on((`dpp`.`id_transaksi` = `ts`.`id_transaksi`))) where (`ts`.`status_pembayaran` = 'Lunas'))) - (select coalesce(sum(`biaya_operasional`.`nominal`),0) from `biaya_operasional`)) - (select coalesce(sum(`gaji_harian_mekanik`.`nominal`),0) from `gaji_harian_mekanik`)) AS `estimasi_laba_bersih` ;
